...
import { Input, Select, Modal, Form, message, Skeleton, Empty } from 'antd';
import {
...
// ─── Add/Edit User Modal ──────────────────────────────────────────────────────
function UserModal({
  open, onClose, editUser, tenantId, allowedRoles,
}: {
  open: boolean;
  onClose: () => void;
  editUser?: User;
  ...
  return (
    <Modal
      ...
      title={
        <div>
          <div style={{ fontWeight: 700, fontSize: 17 }}>{isEdit ? 'Edit User' : 'Add New User'}</div>
          <div style={{ fontSize: 13, color: '#64748b', fontWeight: 400, marginTop: 2 }}>
            {isEdit ? `Editing ${editUser?.first_name} ${editUser?.last_name}` : 'Create a new user account'}
          </div>
        </div>
      }
    >
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <Form.Item label="First Name" name="first_name" rules={[{ required: true, message: 'Required' }]}>
            <Input prefix={<UserOutlined style={{ color: '#94a3b8' }} />} placeholder="First name" />
          </Form.Item>
        ...
        {!isEdit && (
          <>
            <Form.Item label="Email Address" name="email" rules={[{ required: true, type: 'email', message: 'Valid email required' }]}>
              <Input prefix={<MailOutlined style={{ color: '#94a3b8' }} />} placeholder="user@company.com" />
            </Form.Item>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 20, paddingTop: 16, borderTop: '1px solid #f1f5f9' }}>
        <button onClick={() => { onClose(); form.resetFields(); }} style={{ padding: '9px 20px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: 13, color: '#374151' }}>
          Cancel
        </button>
...
export default function UsersPage() {
  ...
  // ── Roles allowed to create depending on who's logged in ──
  const allowedRoles: UserRole[] = isSuperAdmin
    ? ['SUPER_ADMIN', 'SITE_MANAGER', 'FINANCE', 'MAINTENANCE', 'TENANT_ADMIN', 'EMPLOYEE']
    ...
    <div style={{ padding: 24, background: '#f8fafc', minHeight: '100%' }}>
      ...
      <div style={{ ...CARD, padding: '20px 24px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
          <div>
            <h2 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 700, color: '#0f172a' }}>
              {isSuperAdmin ? 'User & Role Management' : 'Team Members'}
            </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
          {[
            { label: 'Total Users',  value: all.length, sub: 'All accounts',     color: '#2563eb', bg: '#eff6ff', icon: '👥' },
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {(q || roleFilter || statusFilter) && (
          <button
            ...
            style={{ padding: '7px 14px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', fontSize: 13, color: '#64748b' }}
          >
            Clear filters
          </button>
        )}
        <div style={{ marginLeft: 'auto', fontSize: 13, color: '#64748b' }}>
          Showing <strong style={{ color: '#0f172a' }}>{isLoading ? '—' : filtered.length}</strong> of {all.length} users
        </div>
      </div>
      ...
      {isError && (
        <div style={{ ...CARD, padding: '40px', textAlign: 'center' }}>
          ...
          <div style={{ fontWeight: 600, color: '#374151', marginBottom: 8 }}>Failed to load users</div>
      ...
      {isLoading && (
        <div style={CARD}>
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} style={{ padding: '14px 20px', borderBottom: i < 5 ? '1px solid #f8fafc' : 'none' }}>
      ...
      {!isLoading && !isError && filtered.length > 0 && (
        <div style={{ ...CARD, overflow: 'hidden' }}>
          ...
          <div style={{
            display: 'grid',
            gridTemplateColumns: isSuperAdmin ? '2.5fr 1.5fr 1fr 1fr 1fr 1fr' : '2.5fr 1.5fr 1fr 1fr 1fr',
            padding: '11px 20px',
            background: '#f8fafc',
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{ width: 36, height: 36, borderRadius: '50%', background: col, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: '#fff', flexShrink: 0 }}>
                    {u.first_name[0]}{u.last_name[0]}
                  </div>
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13, color: '#0f172a' }}>{u.first_name} {u.last_name}</div>
                    <div style={{ fontSize: 11, color: '#94a3b8' }}>ID: {u.id.substring(0, 8)}...</div>
                  </div>
                </div>

                {/* Email */}
                <div style={{ fontSize: 12, color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <MailOutlined style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }} />
                  {u.email}
                </div>
          ...
          <div style={{ padding: '12px 20px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 12, color: '#94a3b8' }}>Showing {filtered.length} of {all.length} users</span>
