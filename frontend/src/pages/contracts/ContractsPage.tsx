...
import { contractApi, tenantApi } from '../../api/services';
...
function NewContractModal({ onClose, tenantId, userId, isSuperAdmin, prefillData }: {
  ...
  const isFromBooking = !!prefillData;
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 620, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, background: '#fff', zIndex: 1, borderRadius: '16px 16px 0 0' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#0f172a' }}>New Lease Contract</h2>
            <p style={{ margin: '2px 0 0', fontSize: 12, color: isFromBooking ? '#2563eb' : '#94a3b8', fontWeight: isFromBooking ? 600 : 400 }}>
              {isFromBooking ? `📋 Pre-filled from booking ${prefillData?.booking_number}` : 'Create a new lease agreement'}
            </p>
          </div>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {isFromBooking && (
            <div style={{ background: 'linear-gradient(135deg,#1e40af,#2563eb)', borderRadius: 12, padding: '16px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <div style={{ width: 42, height: 42, borderRadius: 10, background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>📋</div>
              <div>
                <div style={{ fontWeight: 800, fontSize: 14, color: '#fff', marginBottom: 4 }}>
                  Contract created from Booking {prefillData?.booking_number}
                </div>
                <div style={{ fontSize: 12, color: '#bfdbfe', lineHeight: 1.5 }}>
                  Space: <strong style={{ color: '#fff' }}>{prefillData?.space_name || '—'}</strong><br />
          ...
          {/* Tenant */}
          {isSuperAdmin && (
            <div style={{ background: '#f8fafc', border: `2px solid ${isFromBooking ? '#2563eb' : '#e5e7eb'}`, borderRadius: 10, padding: '14px 16px' }}>
          ...
          {/* Financials — needs to be filled manually */}
          ...
          <div style={{ border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 12 }}>⚙️ Settings</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Auto Renew">
                <Select value={form.auto_renew} onChange={v => setF('auto_renew', v)} style={{ width: '100%' }}
                  options={[{ value: 'false', label: '✗ No — manual renewal' }, { value: 'true', label: '✓ Yes — auto renew' }]} />
              </Field>
            </div>
          </div>
            ...
            <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 10, padding: '14px 18px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                <div style={{ background: '#fff', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: '#64748b', marginBottom: 3 }}>Monthly Rent</div>
                  <div style={{ fontSize: 16, fontWeight: 800, color: '#0f172a' }}>{form.currency} {parseFloat(form.monthly_rent || '0').toLocaleString()}</div>
                </div>
                <div style={{ background: '#fff', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                  <div style={{ fontSize: 10, color: '#64748b', marginBottom: 3 }}>Security Deposit</div>
                  <div style={{ fontSize: 16, fontWeight: 800, color: '#0f172a' }}>{form.currency} {parseFloat(form.deposit_amount || '0').toLocaleString()}</div>
                </div>
                {durationMonths && (
                  <div style={{ background: '#fff', borderRadius: 8, padding: '10px 12px', textAlign: 'center' }}>
                    <div style={{ fontSize: 10, color: '#64748b', marginBottom: 3 }}>Total ({durationMonths}mo)</div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(4px)', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div><h2 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: '#0f172a' }}>Renew Contract</h2><p style={{ margin: '2px 0 0', fontSize: 12, color: '#64748b' }}>{contract.contract_number}</p></div>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: '10px 14px', background: '#f8fafc', borderRadius: 8, fontSize: 13, color: '#64748b' }}>
            Current end: <strong style={{ color: '#0f172a' }}>{formatDate(contract.end_date)}</strong>
          </div>
        ...
        <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 540, boxShadow: '0 24px 64px rgba(0,0,0,0.18)' }}>
          <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <h2 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: '#0f172a' }}>Contract Details</h2>
          ...
          <div style={{ padding: '20px 24px' }}>
            ...
            {daysLeft > 0 && daysLeft <= 30 && contract.status === 'ACTIVE' && (
              <div style={{ background: '#fef3c7', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                ...
                <span style={{ color: '#92400e', fontWeight: 500 }}>Expires in {daysLeft} days{canManage ? ' — consider renewing' : ' — contact your manager'}</span>
              </div>
            )}
            <div style={{ background: '#f8fafc', borderRadius: 10, padding: '12px 16px', marginBottom: 16, display: 'flex', gap: 24 }}>
              <div><div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 500, marginBottom: 2 }}>Monthly Rent</div><div style={{ fontSize: 20, fontWeight: 800, color: '#0f172a' }}>{contract.currency} {parseFloat(contract.monthly_rent).toLocaleString()}</div></div>
              <div style={{ borderLeft: '1px solid #e5e7eb', margin: '0 4px' }} />
              <div><div style={{ fontSize: 10, color: '#94a3b8', fontWeight: 500, marginBottom: 2 }}>Deposit</div><div style={{ fontSize: 20, fontWeight: 800, color: '#0f172a' }}>{contract.currency} {parseFloat(contract.deposit_amount).toLocaleString()}</div></div>
              {contract.auto_renew && <><div style={{ borderLeft: '1px solid #e5e7eb', margin: '0 4px' }} /><div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#059669', fontWeight: 600 }}><RedoOutlined /> Auto-renew</div></>}
            </div>
            {([
              ...
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: '1px solid #f8fafc', fontSize: 13 }}>
                ...
                <span style={{ fontWeight: 600, color: '#0f172a', fontFamily: k === 'Contract #' ? 'monospace' : undefined }}>{v}</span>
              </div>
            ))}
...
function EditContractModal({ contract, onClose, isSuperAdmin }: { contract: LeaseContract; onClose: () => void; isSuperAdmin: boolean }) {
  ...
  const submit = () => { const e = validate(); if (Object.keys(e).length) { setErrors(e); return; } mutation.mutate({ tenant_id: form.tenant_id, start_date: form.start_date, end_date: form.end_date, monthly_rent: parseFloat(form.monthly_rent), deposit_amount: parseFloat(form.deposit_amount), currency: form.currency, payment_due_day: parseInt(form.payment_due_day), auto_renew: form.auto_renew === 'true' }); };
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 480, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', padding: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div><h2 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 800, color: '#0f172a' }}>Edit Contract</h2><p style={{ margin: 0, fontSize: 13, color: '#64748b' }}>Update lease agreement</p></div>
...
export default function ContractsPage() {
  ...
  const canManage     = isSuperAdmin || isSiteManager;
  ...
  // ── THE KEY: auto-open modal when coming from BookingsPage ─────────────────
  useEffect(() => {
    const fromBooking = location.state?.fromBooking;
    if (fromBooking && canManage) {
      setPrefillData(fromBooking);
      setNew(true);
      // Clear state so browser back/refresh doesn't re-open
    ...
    <div style={{ padding: 24, background: '#f8fafc', minHeight: '100%' }}>
      ...
      <div style={{ ...CARD, padding: '20px 24px', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
          <div>
            <h2 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 700, color: '#0f172a' }}>{isTenantAdmin ? 'My Lease Contracts' : 'Lease Contracts'}</h2>
            <p style={{ margin: 0, color: '#64748b', fontSize: 14 }}>{isTenantAdmin ? 'View and sign your lease agreements' : 'Manage all lease agreements and their lifecycle'}</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 14 }}>
          {[
            ...
            { label: 'Draft',        value: draft,            sub: isTenantAdmin && draft > 0 ? '⚠ Needs your signature' : 'Pending signature', color: draft > 0 ? '#d97706' : '#94a3b8', bg: draft > 0 ? '#fffbeb' : '#f8fafc', icon: draft > 0 && isTenantAdmin ? '✍️' : '✏️' },
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        <Input prefix={<SearchOutlined style={{ color: '#94a3b8' }} />} placeholder="Search by contract #..." value={q} onChange={e => setQ(e.target.value)} style={{ width: 240, borderRadius: 8 }} />
        <Select value={statusFilt || 'all'} onChange={v => setStatus(v === 'all' ? '' : v)} style={{ width: 170 }}
          options={[{ value: 'all', label: 'All Status' },{ value: 'DRAFT', label: '✏️ Draft' },{ value: 'ACTIVE', label: '✅ Active' },{ value: 'EXPIRED', label: '⏰ Expired' },{ value: 'TERMINATED', label: '🔒 Terminated' },{ value: 'RENEWED', label: '🔄 Renewed' }]} />
        <div style={{ marginLeft: 'auto', fontSize: 13, color: '#64748b' }}>Showing <strong style={{ color: '#0f172a' }}>{filtered.length}</strong> of {contracts.length}</div>
      </div>

      {isError && <div style={{ ...CARD, padding: '40px', textAlign: 'center' }}><div style={{ fontSize: 36, marginBottom: 12 }}>⚠️</div><div style={{ fontWeight: 600, color: '#374151', marginBottom: 8 }}>Failed to load</div><button onClick={() => refetch()} style={{ padding: '8px 20px', background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer' }}>Retry</button></div>}
      {isLoading && <div style={CARD}>{Array.from({ length: 4 }).map((_, i) => <div key={i} style={{ padding: '16px 20px', borderBottom: i < 3 ? '1px solid #f8fafc' : 'none' }}><Skeleton active paragraph={{ rows: 1 }} /></div>)}</div>}

      {!isLoading && !isError && filtered.length === 0 && (
        <div style={{ ...CARD, padding: '60px', textAlign: 'center' }}>
          <FileTextOutlined style={{ fontSize: 48, color: '#e5e7eb', display: 'block', margin: '0 auto 16px' }} />
          <Empty description={isTenantAdmin ? (contracts.length === 0 ? 'No contracts yet — your manager will send you one.' : 'No matches.') : (contracts.length === 0 ? 'No contracts yet.' : 'No matches.')} />
      ...
      {!isLoading && !isError && filtered.length > 0 && (
        <div style={{ ...CARD, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 0.9fr 1.2fr', padding: '11px 20px', background: '#f8fafc', borderBottom: '1px solid #e5e7eb', fontSize: 11, fontWeight: 600, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          ...
          {filtered.map((c: LeaseContract, i: number) => {
            ...
            return (
              <div key={c.id}
                style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 1fr 0.9fr 1.2fr', padding: '13px 20px', borderBottom: i < filtered.length - 1 ? '1px solid #f8fafc' : 'none', alignItems: 'center', transition: 'background 0.1s', background: needsSign ? '#fefce8' : expiringSoon ? '#fffbeb' : '' }}
              ...
              >
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{c.currency} {parseFloat(c.monthly_rent).toLocaleString()}</div>
                <div style={{ fontSize: 12, color: '#64748b' }}>{c.currency} {parseFloat(c.deposit_amount).toLocaleString()}</div>
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                  <button onClick={() => setSelected(c)} style={{ width: 28, height: 28, borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="View"><EyeOutlined style={{ fontSize: 12, color: '#64748b' }} /></button>
