...
export class ExportController {
  ...
  @Roles(USER_ROLE.SUPER_ADMIN, USER_ROLE.CLIENT_ADMIN, USER_ROLE.MANAGER, USER_ROLE.FINANCE)
  ...
  async exportBookings(
    ...
    const result = await this.exportService.exportBookings(format, dates.from, dates.to, tenantId);
  ...
  async exportInvoices(
    ...
    const dates  = this.parseDates(from, to);
    const result = await this.exportService.exportInvoices(format, dates.from, dates.to, tenantId);
    await this.sendFile(res, result, format);
  }
  ...
  async exportPayments(
    ...
    const dates  = this.parseDates(from, to);
    const result = await this.exportService.exportPayments(format, dates.from, dates.to, tenantId);
    await this.sendFile(res, result, format);
  }
  ...
  async exportMaintenance(
    ...
    const dates  = this.parseDates(from, to);
    const result = await this.exportService.exportMaintenance(format, dates.from, dates.to, tenantId);
    await this.sendFile(res, result, format);
  }
}